...
vec3 apply_vibrance(vec3 color, float intensity) {
    ...
    // Improved vibrance: better preservation of skin tones and natural colors
    float skinTone = smoothstep(0.0, 0.3, color.r - color.g) * smoothstep(0.0, 0.2, color.g - color.b);
    float adjustedIntensity = mix(intensity, max(intensity * 0.7, 1.0), skinTone * 0.5);

    return mix(color, mix(lightness, color, adjustedIntensity), sat);
}

vec3 apply_contrast(vec3 color, float contrast) {
    return max((color - 0.5) * contrast + 0.5, 0.0);
}
