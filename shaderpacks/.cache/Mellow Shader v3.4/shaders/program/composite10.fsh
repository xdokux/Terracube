...
void main() {
    ...
    vec2 Center = texelFetch(colortex2, GlobalPos, 0).rg;
    ...
    if(Center.g > 0) {
        ...
        float eL = texture(colortex2, Coords.xy).r;
        ...
        float eR = texture(colortex2, Coords.zy + ivec2(1, 0) * resolutionInv).r;
    ...
    if(Center.r > 0) {
        ...
        float eL = texture(colortex2, Coords.xy).g;
        ...
        float eR = texture(colortex2, Coords.xz + ivec2(0, 1) * resolutionInv).g;
