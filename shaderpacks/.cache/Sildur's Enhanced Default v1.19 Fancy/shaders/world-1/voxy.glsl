...
void voxy_emitFragment(VoxyFragmentParameters parameters) {
	...
	#ifdef Fog
		fragData0.rgb = mix(fragData0.rgb, fogColor, clamp((length(viewPos) / vxRenderDistance), 0.0, 1.0));
    	//don't scale underwater/effect fog and make slightly darker
		if(isEyeInWater == 1.0 || isEyeInWater == 2.0 || isEyeInWater == 3.0 || darknessFactor > 0.9 || blindness > 0.9) fragData0.rgb = mix(fragData0.rgb, fogColor*0.75, clamp(length(viewPos), 0.0, 1.0));
	#endif    
}