...
void main() {
    ...
    if (isLikelyWater > 0.5) {
        // Flat, saturated toon-water blue rather than a realistic tint.
        albedo.rgb = mix(albedo.rgb, vec3(0.25, 0.55, 0.85), 0.55);
    }
    ...
    if (isEyeInWater == 1) {
        ...
        litColor = mix(litColor, vec3(0.15, 0.4, 0.55), fogAmount);
    } else {
