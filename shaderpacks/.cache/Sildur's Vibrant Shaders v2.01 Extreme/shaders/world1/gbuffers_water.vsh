...
vec3 funShadows(vec3 shadowPos, vec3 normal) {
	...
	if (diffuse > 0.001) {
		...
		if (abs(shadowPos.x) < 1.0-1.5/shadowMapResolution && abs(shadowPos.y) < 1.0-1.5/shadowMapResolution && abs(shadowPos.z) < 6.0){  //only if on shadowmap
			float pdepth = 2.2;	//fallback if PCSS shadows are disabled, default is 1.412
...
void main() {
    ...
    vReflData[0] = viewPos;
    vReflData[1] = clipPos.xyz;
    vReflData[2] = vec3(gbufferProjection[0].x, gbufferProjection[1].y, gbufferProjection[2].z);
#endif
	...
	vec3 emissiveLightC = vec3(emissive_R,emissive_G,emissive_B) * 10.0;
	
	float NdotL = dot(normal.xyz,sunVec);
	float NdotU = dot(normal.xyz,upVec);
	
	const vec3 moonlight = vec3(0.0075, 0.0135, 0.027); //supposed to scale with define Moonlight but super bright on translucent blocks
	...
	if(mc_Entity.x == ENTITY_ICEGLASS) normal.a = 0.9;
...
/*
	#ifdef Shadows
		vertexShadowPos = funShadows(position.xyz, normal.xyz);
	#endif
*/
}