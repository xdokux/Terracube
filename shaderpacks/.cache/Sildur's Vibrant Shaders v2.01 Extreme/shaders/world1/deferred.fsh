...
vec3 funFog(vec3 albedo, vec3 skyC, vec3 fragpos, float skyL) {
	...
	//#ifdef defskybox
		vec3 fogC = mix(fogColor * 0.5, albedo.rgb, 0.5) * 0.1;
	//#endif
...
void main() {
	...
	vec3 emissiveLightC = vec3(emissive_R,emissive_G,emissive_B) * 10.0; //boost emissive lighting in end
...
*/
	...
	//#else
		//float dif = translucent? dot(normal, normalize(shadowLightPosition)) * 0.35 + 0.65 : clamp(dot(normal, normalize(shadowLightPosition)),0.0,1.0);
			 ...
			 // dif *= 0.5;
		float dif = 0.0;
		vec3 finalLight = (dif*LightC*(SkyL2*skyL)*2.15+ambientC*(1.0/(SkyL2*skyL*0.5+0.5))*1.4)*0.1;
		albedo.rgb *= finalLight;
	//#endif
	...
	//apply fog to sky aswell in End
	#ifdef Fog
		//undithered sky for fog / land
		skyC = pow(max(skyC, 0.0), vec3(1.4));
		albedo.rgb = funFog(albedo.rgb, skyC, fragpos0, skyL);
	#endif
