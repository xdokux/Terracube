...
void main() {
	...
	vec3 sunlight = sunlightColor * clamp(dot(worldLightVector, normal), foliage, 1.0) * lightmap.g * shadow * mix(0.25, 1.0, 1.0 - rainStrength);
	...
	color.rgb += sunlightColor * specular * shadow * strength * (1.1 - rainStrength) * clamp(dot(worldLightVector, normal), foliage, 1.0);
}