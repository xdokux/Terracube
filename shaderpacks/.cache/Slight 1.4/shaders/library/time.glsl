...
const float times[9] = float[](
    0.0, 0.05, 0.25, 0.45, 0.5, 0.55, 0.75, 0.95, 1.0
);
...
const vec3 skyTopColor[5] = vec3[](
    ...
    vec3(0.1, 0.11, 0.2) * 1.0,  
    vec3(0.04, 0.05, 0.1) * 1.0  
);

const vec3 skyMiddleColor[5] = vec3[](
    vec3(0.35, 0.57, 0.5) * 2.0,
    vec3(0.6, 0.9, 1.2) * 1.5,
    vec3(0.7, 0.6, 0.6) * 1.4,
    vec3(0.4, 0.3, 0.5) * 0.5,
    vec3(0.07, 0.09, 0.1) * 3.0
);
...
const vec3 skyHorizonColor[5] = vec3[](
    ...
    vec3(1.0, 0.8, 0.9) * 0.4,
    vec3(1.0, 1.0, 1.0) * 0.2
);

const vec3 skyLightColor[5] = vec3[](
    vec3(0.15, 0.17, 0.2) * 3.0,
    vec3(0.3, 0.4, 0.6) * 1.0,
    vec3(0.13, 0.1, 0.2) * 1.0,
    vec3(0.15, 0.1, 0.2) * 0.8,
    vec3(0.0, 0.08, 0.25) * 0.4
);

const vec3 pointLightColor[5] = vec3[](
    vec3(3.0, 1.7, 0.5) * 0.3 * 2.0,
    vec3(2.0, 1.5, 1.0) * 0.4 * 2.0,
    vec3(1.0, 0.25, 0.0) * 0.8 * 2.0,
    vec3(1.3, 1.1, 0.7) * 0.1 * 2.0,
    vec3(0.2, 0.2, 0.2) * 0.3 * 2.0
);
...
float getPointLightBlackout() {
    ...
    float r1 = clamp(1.0 - abs(t - 0.5) / 0.004, 0.0, 1.0);
    float r2 = clamp(1.0 - abs(t - 0.0) / 0.004, 0.0, 1.0);
