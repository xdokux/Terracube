...
vec3 viewToScreenSpace(vec3 pos, const bool taaAware) {
    return viewToScreenSpace(pos, gbufferProjection, taaAware);
}
vec3 viewToScreenSpace(vec3 pos) {
    return viewToScreenSpace(pos, gbufferProjection, true);
}
