...
void main() {
	...
	vec4 shadowClip = shadowProjection * shadowModelView * vec4(worldPosVar, 1.0);
	...
	// Cheap sun-glint specular: a tight highlight around the reflection
	// direction, gated by the shadow map so it disappears when the sun is
	// actually blocked rather than just shining through unoccluded water.
	vec3 halfway = normalize(lightDir + viewDir);
	float spec = pow(max(dot(normal, halfway), 0.0), 120.0) * shadow * skyAccess;

	vec3 ambient = cubyz_ambientLight * 0.5 + lightmapColor * 0.5;
	vec3 lit = albedo.rgb * (ambient + NdotL * shadow * skyAccess * 0.8) + vec3(spec) * 1.5;
