...
void main() {
    ...
    // Attenuate the effect on pixels that are themselves sky. Real
    // crepuscular rays are visible where light is scattering onto
    // foreground surfaces/fog - adding more brightness directly onto
    // already-bright sky pixels is what turned the whole sky white.
    float selfDepth = texture2D(depthtex0, texcoord).r;
    float selfIsSky = step(0.9999, selfDepth);
    float skyAttenuation = mix(1.0, 0.25, selfIsSky);
