...
void main() {
	...
	vec3 sunlight = getPointLightColor() * clamp(dot(worldLightVector, normal) * 0.85 + 0.15, 0.1, 1.0) * shadow * mix(0.25, 1.0, 1.0 - rainStrength) * lightmap.g;
	
	float specular = pow(max(dot(normal, normalize(worldLightVector - normalize(feetPlayerPos))), 0.0), 6.0) * lightmap.g;
	...
	color.rgb += getPointLightColor() * specular * shadow * strength * (1.1 - rainStrength) * clamp(dot(worldLightVector, normal), 0.1, 1.0);
}