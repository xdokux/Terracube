...
void main() {
	...
	vec3 skylight = lm.g * getSkyLightColor();
	vec3 blocklight = lm.r * blocklightColor;
	vec3 ambient = ambientColor + vec3(0.2, 0.22, 0.25) * nightvis;

	vec3 sunlight = getPointLightColor() * clamp(dot(worldLightVector, worldNormal), 0.2, 1.0) * shadow * mix(0.25, 1.0, 1.0 - rainStrength) * lm.g;

	color.rgb *= blocklight + skylight + ambient + sunlight;

	vec3 worldViewDir = normalize(feetPlayerPos);
	vec3 worldHalfVec = normalize(worldLightVector - worldViewDir);
	float specular = pow(max(dot(worldNormal, worldHalfVec), 0.0), 1000.0) * lm.g;

	packLight = vec4(lm, 0.0, 1.0);
	packNormal = vec4(worldNormal * 0.5 + 0.5, 1.0);
	packMask = vec4(mask, 0.0, 0.0, 1.0);
	packSpecular = vec4(getPointLightColor() * specular * shadow * (1.1 - rainStrength), 1.0);
}