...
void main() {
	...
	vec3 sunlight = sunlightColor * clamp(dot(worldLightVector, normal) * 0.85 + 0.15, 0.1, 1.0) * shadow * lightmap.g;
	...
	color.rgb += sunlightColor * specular * shadow * strength * clamp(dot(worldLightVector, normal), 0.1, 1.0);
}