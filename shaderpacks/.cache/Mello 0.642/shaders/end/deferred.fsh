...
void main() {
	...
	vec3 sunlight = sunlightColor * clamp(dot(worldLightVector, normal) * 0.85 + 0.15, foliage, 1.0) * shadow * lightmap.g;
	...
	color.rgb += sunlightColor * specular * shadow * strength * clamp(dot(worldLightVector, normal), foliage, 1.0);
}