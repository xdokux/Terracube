...
void main() {
    ...
    vec3 skyColor = isEyeInWater == 1
    ? vec3(0.0)
    : calculateSkyGradient(reflDir) + calculateMie(reflDir);
    ...
    float fogFactor = exp(-5.0 * (1.0 - length(viewPos) / far));
