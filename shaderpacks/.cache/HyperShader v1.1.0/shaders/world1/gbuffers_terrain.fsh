...
/* gbuffers_terrain.fsh (END) — VANILLA (lightmap vanilla, sin sombras ni grano).
   Mantiene la lava emisiva por si acaso.
   v1.1: luz dinámica de la antorcha en mano. */
...
void main() {
    ...
    vec3 lit;
    if (matID == 0.25) {
        lit = lavaFromTexture(albedo.rgb);     // lava tipo Solas
    } else {
