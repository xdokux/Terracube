...
varying vec4 tint;
		...
		return clamp(
	...
	#if END_PORTAL_BACKGROUND_END == 2
		#ifdef ENDER_NEBULAE
			...
			vec4 drawClouds(vec2 pos) {
				...
				if (noise > 0.0) { //there are clouds here
					return vec4(
						mix(
							cloudIlluminationColorWhenSunny,
							cloudBaseColorDuringSunnyDays,
							fogify(noise - colorNoise, 0.25)
						),
						1.0 - fogify(noise, 0.0625)
					);
				}
				return vec4(0.0);
			}
...
void main() {
	vec4 color = texture2D(texture, texcoord) * tint;
		...
		if (blockEntityId == 119) {
			...
			#if END_PORTAL_BACKGROUND_END == 1 //overworld fog color
				color = vec4(0.75, 0.875, 1.0, 1.0);
			#elif END_PORTAL_BACKGROUND_END == 2 //end background
			...
			#elif END_PORTAL_CLOUDS_END == 2 //void clouds
