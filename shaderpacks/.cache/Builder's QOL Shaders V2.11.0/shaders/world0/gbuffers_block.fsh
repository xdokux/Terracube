...
void main() {
	vec4 color = texture2D(texture, texcoord) * tint;
		...
		if (blockEntityId == 119) {
			...
			#if END_PORTAL_BACKGROUND_OVERWORLD == 1 //overworld fog color
				color = vec4(0.75, 0.875, 1.0, 1.0);
			#elif END_PORTAL_BACKGROUND_OVERWORLD == 2 //end background
			...
			#if END_PORTAL_FOREGROUND_OVERWORLD == 1 //overworld screenshot
				vec4 islandImage = texture2D(gaux1, backgroundPos * 0.25 + 0.5);
				color.rgb = mix(color.rgb, islandImage.rgb, islandImage.a);
			#elif END_PORTAL_FOREGROUND_OVERWORLD == 2 //end screenshot
				vec4 islandImage = texture2D(gaux2, backgroundPos + 0.5);
				color.rgb = mix(color.rgb, islandImage.rgb, islandImage.a);
			#endif

			#if END_PORTAL_CLOUDS_OVERWORLD == 1 //overworld clouds
			...
			#elif END_PORTAL_CLOUDS_OVERWORLD == 2 //void clouds
