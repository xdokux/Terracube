...
void main() {
	...
	float bias = max(0.0005 * (1.0 - dot(normalize(shadowLightPosition), worldNormal)), 0.0002);
	...
	vec3 sunlight = getPointLightColor() * clamp(dot(worldLightVector, worldNormal), 0.2, 1.0) * lmcoord.g * shadow * mix(0.25, 1.0, 1.0 - rainStrength);
