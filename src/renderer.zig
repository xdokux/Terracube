...
const c = @import("c");
/// Shaderpack pipeline hooks. Generated by build.zig; every call is a no-op when no renderer-side
/// mod is installed or no pack is loaded, so these need no guards at the call sites.
/// Public so the shader settings screen can ask the mod which packs are installed. Every other
/// hook is called from this file alone.
pub const renderhook = @import("renderhook");

/// One configurable option of the loaded shaderpack, as the settings screen sees it.
///
/// Declared here rather than in the mod because it is the *interface* between them, and the
/// generated no-mod hook has to name the type in its signature whether or not the mod exists.
///
/// Every string is owned by whoever passed the allocator to `listPackOptions`, including the
/// `values` slice itself. `free` releases one entry.
pub const ShaderPackOption = struct {
	name: []const u8,
	/// The value in effect: the user's override where there is one, otherwise what the pack's own
	/// source declares.
	value: []const u8,
	/// The values the option accepts, in the order the pack lists them. Booleans get true/false.
	values: []const []const u8,
	/// Whether the user has set this, as opposed to it being the pack's default.
	overridden: bool = false,

	pub fn free(self: ShaderPackOption, allocator: main.heap.NeverFailingAllocator) void {
...
/// Public so the shaderpack pipeline can read the lit world before the fog and tonemap pass,
/// which is the closest equivalent to what Minecraft's gbuffers stage leaves behind.
pub var worldFrameBuffer: graphics.FrameBuffer = undefined;
...
pub fn updateViewport(width: u31, height: u31) void {
	...
	renderhook.updateSize(lastWidth, lastHeight);
}
...
/// Near end of the depth range the world is drawn with.
///
/// Cubyz reserves the first thousandth for the held item, and leaves the range set. A shaderpack's
/// shadow maths assumes GL's default `[0, 1]` — it compares a depth it computed itself against
/// what the shadow pass stored — so the bridge resets the range for that pass and restores this
/// afterwards. Named rather than repeated as a literal so the two cannot drift apart.
pub const worldDepthRangeNear: f64 = 0.001;
...
pub fn renderWorld(world: *World, ambientLight: Vec3f, skyColor: Vec3f, playerPos: Vec3d) void { // MARK: renderWorld()
	...
	// Deliberately *after* `updateViewMatrix`, and given the same `playerPos` the geometry is drawn
	// with. A shaderpack's temporal effects compare this frame's camera against last frame's, so
	// capturing the snapshot before the camera updates hands it a matrix one frame stale relative to
	// what it is shading — which reads as ghosting that scales with how fast you turn. Sampling the
	// player position independently has the same effect: it is a mutex-guarded read the physics
	// thread also writes, so a second sample lands somewhere slightly different from the one the
	// chunks were positioned against, and the mismatch changes every frame.
	renderhook.beginFrame(@floatCast(main.lastDeltaTime.load(.monotonic)), playerPos);
	...
	// **Iris's order is shadow -> prepare -> gbuffers -> deferred -> composite -> final**, and the
	// first two come *before* the camera draws anything. The shadow map renders the chunk geometry
	// from the light's viewpoint; the `prepare` passes then lay down whatever the pack wants under
	// its terrain — Nostalgia's `prepare1` writes its sky and sun disc into colortex0 and expects
	// geometry to paint over it. Running them after geometry instead paints the sky across the
	// albedo; not running them at all leaves the sky pure black. Both return immediately with no
	// pack, and both leave their own framebuffer bound, so Cubyz's is restored afterwards for the
	// path where the pack has no usable terrain program and the engine draws terrain itself.
	renderhook.renderShadowMap(&chunkLists, ambientLight, playerPos);
	renderhook.runPreparePasses(main.Window.width, main.Window.height);
	worldFrameBuffer.bind();
	c.glViewport(0, 0, lastWidth, lastHeight);

	gpu_performance_measuring.startQuery(.chunk_rendering);
	// Routes the opaque chunk draw through the pack's own `gbuffers_terrain` when one compiled. The
	// override swaps only the *program*, leaving Cubyz's raster state and buffers alone, so this is
	// the same draw call either way. Returns false with no pack loaded, or when the pack's terrain
	// program failed to compile — in which case the frame degrades to Cubyz's own terrain shading
	// underneath the pack's post chain.
	const shaderpackTerrain = renderhook.beginTerrain();
	defer if (shaderpackTerrain) renderhook.endTerrain();
	chunk_meshing.drawChunksIndirect(&chunkLists, game.projectionMatrix, ambientLight, playerPos, false);
	gpu_performance_measuring.stopQuery();

	// Everything from here down to the selection outline still draws with Cubyz's own shaders, which
	// write a single output into a framebuffer a shaderpack may have given four attachments — and GL
	// leaves every attachment a shader does not name *undefined*, not untouched. Narrowing the draw
	// buffer list to the albedo target keeps the pack's material and normal buffers intact; see
	// `beginCubyzShadedDraws`. Safe to call unconditionally: it returns immediately with no pack.
	renderhook.beginCubyzShadedDraws();
	...
	// With a shaderpack loaded the selection outline is drawn *after* the pack's post chain instead,
	// further down. It writes pure black, and a pack that derives exposure from the scene reads that
	// black as "this region is unlit" — see the call site below.
	if (!shaderpackTerrain) MeshSelection.render(game.projectionMatrix, game.camera.viewMatrix, playerPos);

	renderhook.endCubyzShadedDraws();
...

	{
		...
		// Translucent chunks are the same SSBO geometry as the opaque ones, so a shaderpack draws
		// them with its own `gbuffers_water` through the same generated prologue.
		const shaderpackTranslucent = renderhook.beginTranslucent();
		defer if (shaderpackTranslucent) renderhook.endTranslucent();
		chunk_meshing.drawChunksIndirect(&chunkLists, game.projectionMatrix, ambientLight, playerPos, true);
		gpu_performance_measuring.stopQuery();
	}

	c.glDepthRange(0, worldDepthRangeNear);
	itemdrop.ItemDropRenderer.renderDisplayItems(ambientLight, playerPos);
	c.glDepthRange(worldDepthRangeNear, 1);
	...
	// The shaderpack chain runs here: after Cubyz's own deferred pass, so it has the finished
	// world to work from, and before the HUD, so the interface is not post-processed.
	renderhook.runPostChain(worldFrameBuffer.frameBuffer, main.Window.width, main.Window.height);

	// The selection outline, drawn onto the *finished* frame so it takes no part in any pack
	// calculation: Kappa's local auto-exposure read its pure black as an unlit region and bloomed a
	// white blob around it. That needs the scene's depth stamped back into the default framebuffer
	// first — otherwise every edge of the cube passes the depth test, including the six inside the
	// block, and the result reads as an X-ray wireframe.
	if (shaderpackTerrain) {
		renderhook.restoreSceneDepth();
