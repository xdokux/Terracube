...
/// How many block types the current world registered.
///
/// Exposed for mods that need to walk the whole registry — the shaderpack bridge builds its
/// `mc_Entity` table by matching every block's id against the pack's `block.properties`.
