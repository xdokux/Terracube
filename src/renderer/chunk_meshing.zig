...
pub var commandUniforms: struct {
	...
	lodDistance: c_int,
	/// The two culls the shadow pass has to bypass. Both are declared in
	/// `fillIndirectBuffer.comp` with the reasoning; both filter against the **camera**, which is
	/// the wrong viewpoint for a shadow map. Set explicitly on every path rather than left to
	/// whatever the last dispatch happened to use, since an unset `bool` uniform reads false and
	/// that silently starves the shadow map instead of erroring.
	ignoreVisibility: c_int,
	ignoreDirection: c_int,
} = undefined;
...
/// Replaces the terrain program with a shaderpack's `gbuffers_terrain`, leaving Cubyz's raster
/// state untouched.
///
/// Only the program is swapped: `pipeline.bind` is still what configures depth testing, culling
/// and blending, so pack geometry rasterises exactly as Cubyz's own does. The pack program reads
/// the same SSBOs through a generated prologue, which is why no vertex data has to change.
pub const TerrainOverride = struct {
	program: c_uint,
	bindUniforms: *const fn (projMatrix: Mat4f, ambient: Vec3f, playerPos: Vec3d) void,
};

pub var terrainOverride: ?TerrainOverride = null;

/// The same substitution for the transparent pass, which a shaderpack draws with `gbuffers_water`.
///
/// Separate from `terrainOverride` because the two passes are swapped in and out at different
/// points in the frame and use different pack programs, even though both run over the same SSBO
/// geometry through the same generated prologue.
pub var transparentOverride: ?TerrainOverride = null;

pub fn bindShaderAndUniforms(projMatrix: Mat4f, ambient: Vec3f, playerPos: Vec3d) void {
	pipeline.bind(null);

	if (terrainOverride) |override| {
		c.glUseProgram(override.program);
		override.bindUniforms(projMatrix, ambient, playerPos);
	} else {
...
pub fn bindTransparentShaderAndUniforms(projMatrix: Mat4f, ambient: Vec3f, playerPos: Vec3d) void {
	...
	if (transparentOverride) |override| {
		c.glUseProgram(override.program);
		// Unlike the opaque override, this one has to replace the blend state as well as the
		// program. Cubyz's transparent pipeline blends against a *second* fragment output — its
		// shader emits a per-channel blend factor as colour index 1, which is what
		// `dstColorBlendFactor = src1Color` reads. A shaderpack program has no such output, so
		// leaving that state in place would blend against an undefined source.
		//
		// The replacement comes from the pack's own `blend.*` declarations and is issued inside
		// `bindUniforms`, which runs after this. That is safe because `Pipeline.bind` above sets
		// every piece of GL state unconditionally rather than caching it, so Cubyz's own blending
		// returns with the next bind.
		override.bindUniforms(projMatrix, ambient, playerPos);
		vao.bind();
		return;
	}
...
pub const ChunkLists = [main.settings.highestSupportedLod + 1]main.ListManaged(u32);

/// Draws chunks for a shadow map: the opaque meshes of every chunk listed and then, when
/// `beforeTranslucent` is given, their transparent meshes as well.
///
/// Deliberately *not* `drawChunksIndirect`: that runs occlusion queries which write each chunk's
/// visibility state back into the chunk buffer. Running them a second time from the light's point
/// of view would overwrite what the camera pass computed, and the next frame would cull geometry
/// against the wrong viewpoint. The lists are the caller's: the camera's set is frustum-culled and
/// omits every occluder beside and behind the viewer, so the shadow pass enumerates its own.
///
/// The transparent meshes come second, the order Minecraft's shadow pass draws its translucent
/// layer in, and `beforeTranslucent` runs between the two with the framebuffer still bound - the
/// caller copies the opaque-only depth out there, which is what a shaderpack's `shadowtex1` holds.
/// They are drawn in whatever order the camera's last sort left them, which is enough for depth and
/// is also what Minecraft does (`ShadowRenderer.java:532-537`, "they won't be sorted in the normal
/// rendering pass"). Both layers go through `terrainOverride`, as Iris keys every terrain layer of
/// the shadow pass to its one `shadow` program (`MixinGameRenderer.java:145`).
pub fn drawChunksForShadow(chunkIds: *const ChunkLists, projMatrix: Mat4f, ambient: Vec3f, playerPos: Vec3d, beforeTranslucent: ?*const fn () void) void {
	for (0..chunkIds.len) |lod| {
		const chunkIDs = chunkIds[lod].items;
		...
		c.glUniform1i(commandUniforms.isTransparent, 0);
		...
		// The light is not the camera. Without both of these the shadow map contains only what the
		// camera could already see, and loses the up-facing group of every chunk above the player —
		// measured at 22% coverage against 69% with them.
		c.glUniform1i(commandUniforms.ignoreVisibility, 1);
		c.glUniform1i(commandUniforms.ignoreDirection, 1);
	...
	const translucentHook = beforeTranslucent orelse return;
	translucentHook();
	for (0..chunkIds.len) |lod| {
		const chunkIDs = chunkIds[lod].items;
		if (chunkIDs.len == 0) continue;
		...
		// One command per chunk: a transparent mesh is drawn whole, with no per-direction groups.
		...
		c.glUniform3i(commandUniforms.playerPositionInteger, @floor(playerPos[0]), @floor(playerPos[1]), @floor(playerPos[2]));
		c.glUniform1i(commandUniforms.onlyDrawPreviouslyInvisible, 0);
		// The bypass matters more here than for the opaque loop. The compute's transparent branch
		// reads and then resets the visibility state the camera's own transparent draw consumes
		// later in the frame, and this runs before the camera's occlusion test has set it.
		c.glUniform1i(commandUniforms.ignoreVisibility, 1);
		c.glUniform1i(commandUniforms.ignoreDirection, 1);
		c.glDispatchCompute(@intCast(@divFloor(chunkIDs.len + 63, 64)), 1, 1);
		c.glMemoryBarrier(c.GL_SHADER_STORAGE_BARRIER_BIT | c.GL_COMMAND_BARRIER_BIT);

		// The opaque pipeline's state on purpose: depth writes stay on, as Minecraft's translucent
		// layer writes depth in the shadow pass too, and the override's own binder adds the blending
		// its program wants.
		bindShaderAndUniforms(projMatrix, ambient, playerPos);
		c.glBindBuffer(c.GL_DRAW_INDIRECT_BUFFER, commandBuffer.ssbo.bufferID);
		c.glMultiDrawElementsIndirect(c.GL_TRIANGLES, c.GL_UNSIGNED_INT, @ptrFromInt(allocation.start*@sizeOf(IndirectData)), drawCallsEstimate, 0);
...
fn drawChunksOfLod(chunkIDs: []const u32, projMatrix: Mat4f, ambient: Vec3f, playerPos: Vec3d, transparent: bool) void {
	...
	c.glUniform3i(commandUniforms.playerPositionInteger, @floor(playerPos[0]), @floor(playerPos[1]), @floor(playerPos[2]));
	// Explicitly 0 on the camera path. The shadow pass sets both to 1 and runs earlier in the frame,
	// so leaving these unset would carry its bypass onto the hot path and disable occlusion culling
	// for the whole world — which costs frame rate rather than correctness, and so would not show up
	// as anything but "the game got slower".
	c.glUniform1i(commandUniforms.ignoreVisibility, 0);
	c.glUniform1i(commandUniforms.ignoreDirection, 0);
	if (!transparent) {
		c.glUniform1i(commandUniforms.onlyDrawPreviouslyInvisible, 0);
