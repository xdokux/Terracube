...
/// Structure map generators contributed by mods, from `mods/<mod>/structuremapgen/*.zig`.
pub const mod_structure_map_generators = @import("structuremapgen");
...
pub const StructureMapGenerator = struct {
	...
	fn registryEntry(comptime Generator: type) struct { []const u8, StructureMapGenerator } {
		return .{Generator.id, .{
			.init = &Generator.init,
	...
	const generatorRegistry: std.StaticStringMap(StructureMapGenerator) = .initComptime(blk: {
		...
		const modDecls = @typeInfo(mod_structure_map_generators).@"struct".decls;
		var generators: [decls.len + modDecls.len]struct { []const u8, StructureMapGenerator } = undefined;
		for (0..decls.len) |i| {
			generators[i] = registryEntry(@field(structure_map_generators, decls[i].name));
		}
		for (0..modDecls.len) |i| {
			generators[decls.len + i] = registryEntry(@field(mod_structure_map_generators, modDecls[i].name));
		}
