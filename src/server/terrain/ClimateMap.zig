...
/// Climate map generators contributed by mods, from `mods/<mod>/climategen/*.zig`.
pub const mod_climate_generators = @import("climategen");
...
pub const ClimateMapGenerator = struct {
	...
	fn registryEntry(comptime Generator: type) struct { []const u8, ClimateMapGenerator } {
		return .{Generator.id, .{
			.init = &Generator.init,
	...
	const generatorRegistry: std.StaticStringMap(ClimateMapGenerator) = .initComptime(blk: {
		...
		const modDecls = @typeInfo(mod_climate_generators).@"struct".decls;
		var generators: [decls.len + modDecls.len]struct { []const u8, ClimateMapGenerator } = undefined;
		for (0..decls.len) |i| {
			generators[i] = registryEntry(@field(climate_generators, decls[i].name));
		}
		for (0..modDecls.len) |i| {
			generators[decls.len + i] = registryEntry(@field(mod_climate_generators, modDecls[i].name));
		}
