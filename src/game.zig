...
pub const Freecam = struct { // MARK: Freecam
	/// Whether the freecam is currently active. While active the real player entity is
	/// frozen in place (no physics, no input-driven movement) and the render/stream camera
	/// is instead driven by `pos`, which is moved independently using the normal movement keys.
	pub var enabled: bool = false;
	pub var pos: Vec3d = .{0, 0, 0};

	/// Adjustable from the freecam settings window (opened via keybind).
	pub var flySpeed: f64 = 43.0; // Matches the creative "hyperSpeed" flying speed.
	pub var sprintMultiplier: f64 = 3.0;
	...
	fn update(deltaTime: f64) void {
		if (!enabled) return;
		if (!main.Window.grabbed) return; // Don't fly around while a menu is open.

		// Re-use the same camera rotation the player normally looks through, so toggling
		// freecam on/off never causes the view to snap to a different orientation.
		...
		const forward: Vec3d = @floatCast(camera.direction);

		const forwardValue: f64 = @floatCast(KeyBoard.key("forward").value);
		const backwardValue: f64 = @floatCast(KeyBoard.key("backward").value);
		const leftValue: f64 = @floatCast(KeyBoard.key("left").value);
		const rightValue: f64 = @floatCast(KeyBoard.key("right").value);

		var movementDir: Vec3d = .{0, 0, 0};
		movementDir += forward*@as(Vec3d, @splat(forwardValue));
		movementDir -= forward*@as(Vec3d, @splat(backwardValue));
		movementDir += right*@as(Vec3d, @splat(leftValue));
		movementDir -= right*@as(Vec3d, @splat(rightValue));
		if (KeyBoard.key("jump").pressed) movementDir[2] += 1;
		if (KeyBoard.key("fall").pressed) movementDir[2] -= 1;

		if (vec.lengthSquare(movementDir) > 0.0001) {
			if (vec.lengthSquare(movementDir) > 1.0) movementDir = vec.normalize(movementDir);
			var speed: f64 = flySpeed;
			if (KeyBoard.key("sprint").pressed) speed *= sprintMultiplier;
			pos += movementDir*@as(Vec3d, @splat(speed*deltaTime));
		}
	}
};

pub const Zoom = struct { // MARK: Zoom
	var currentFov: f32 = 0; // Lazily initialized to settings.fov on first update.
	var zoomLevel: f32 = 4.0; // How many times the fov is divided by when fully zoomed in.

	const minZoomLevel: f32 = 1.5;
	const maxZoomLevel: f32 = 40.0;
	const smoothing: f32 = 12.0; // Higher = snappier transition in/out of zoom.
	const scrollStep: f32 = 0.15; // Fractional zoom-level change per scroll notch.

	fn isZooming() bool {
		return main.Window.grabbed and KeyBoard.key("zoom").pressed;
	}

	fn update(deltaTime: f64) void {
		if (currentFov == 0) currentFov = settings.fov;
		const zooming = isZooming();

		if (zooming and main.Window.scrollOffsetInteger != 0) {
			// While actively zoomed, scrolling changes how far in we're zoomed rather than the hotbar slot.
			const notches: f32 = @floatFromInt(main.Window.scrollOffsetInteger);
			zoomLevel = std.math.clamp(zoomLevel*std.math.pow(f32, 1.0 + scrollStep, notches), minZoomLevel, maxZoomLevel);
			main.Window.scrollOffsetInteger = 0; // Consume it so it doesn't also scroll the hotbar this frame.
		}

		const target: f32 = if (zooming) settings.fov/zoomLevel else settings.fov;
		const t: f32 = 1 - @exp(-smoothing*@as(f32, @floatCast(deltaTime)));
		currentFov += (target - currentFov)*t;
	}

	pub fn getFov() f32 {
		if (currentFov == 0) return settings.fov;
		return currentFov;
	}

	/// Scales mouse-look sensitivity down while zoomed in, so aiming stays precise at high zoom levels.
	pub fn getSensitivityScale() f32 {
		if (settings.fov <= 0) return 1;
		return std.math.clamp(getFov()/settings.fov, 1.0/maxZoomLevel, 1.0);
	}
};
...
pub fn update(deltaTime: f64) void { // MARK: update()
	...
	if (Freecam.enabled) {
		// The real player entity is paused in place: no physics, no collision, no
		// input-driven movement. This keeps the freecam purely client-side — the server
		// still only ever sees the frozen position — while letting the camera roam freely.
		Player.super.vel = .{0, 0, 0};
